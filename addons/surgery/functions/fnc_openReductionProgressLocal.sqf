...
/*
 * Author: Mazinski
 * Local call to progress the treatment process for a complex fracture.
 *
 * Arguments:
 * 0: Medic <OBJECT>
 * 1: Patient <OBJECT>
...
if (_number == _liveFracture) exitWith {
    switch (_entry) do {
        case (2.1):{
            _surgeryString = LLSTRING(EXPOSED);
        };
        case (2.3):{
            _surgeryString = LLSTRING(IRRIGATED);
        };
        case (3.3):{
            _surgeryString = LLSTRING(CLAMPED);
        };
    };
